...
from report_cards.serializers import AbsenceReportCardSerializer, ListReportCardSerializer
    ...
    def update(self, request, *args, **kwargs):
        ...
        if(request.user.role == 'TEACHER'):
            ...
            if subject not in teacher.Subjects.all():
                return Response({"detail": "Subject does not belong to this teacher"}, status.HTTP_403_FORBIDDEN)
        

        report_card = get_object_or_404(ReportCard, student=student, subject=subject)
        ...
        return Response({"detail": f"Absences registered successfully. Total student absences: {report_card.absences}"}, status.HTTP_200_OK)
